...
let url_hard = "findmine_hard.html";
...
const flag = document.querySelectorAll("#flag_real");
back.onclick = notice;

let win = new Array(rows);
for (let i=0; i<cols; i++){
    win[i] = new Array(cols).fill(1);
}
    ...
    cell.addEventListener("click", function(){
        if (check){
            ...
            if (arr[i][j] == 0){
                ...
                cell.style.background = "linear-gradient(90deg, rgb(210, 16, 16), rgb(150, 16, 16))";
                lose();
            }
            else {
...
level_easy.addEventListener("contextmenu", function(event){
